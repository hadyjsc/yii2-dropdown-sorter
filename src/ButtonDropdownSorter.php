...
class ButtonDropdownSorter extends Widget
{
    ...
    /**
     * @var array HTML for link options
     * e.g class, tabindex and other
     */
    public $linkOptions = [];
    /**
    ...
    protected function renderSortButtonDropdown()
    {
        ...
        foreach ($attributes as $name) {
        	$links[] = Html::tag('li', $this->sort->link($name, $this->linkOptions ));
        }

        if (empty($this->label))
        	$this->label = 'Sort';

		return \jscdev\bs4\ButtonDropdown::widget([
			'label' => $this->label,
			'dropdown' => [
				'items' => $links,
			],
